...
import { BackSide, Color, Texture } from 'three'
...
const EnvironmentShaderMaterialExtended = extend(EnvironmentShaderMaterial)
...
const Environment: FC = () => {
  ...
  const shaderMaterial = useRef<typeof EnvironmentShaderMaterialExtended & Uniforms>(null)
  ...
  return (
    <EnvironmentDrei background={true} ground={false} frames={Infinity} far={8.5} resolution={resolution}>
      <Sphere position={[0, 0, 0]} args={[8, 24, 24]}>
        <EnvironmentShaderMaterialExtended
          ref={shaderMaterial}
