...
const ParticleAnimationShader = extend(ParticleAnimationShaderMaterial)
...
const SimulationShader = extend(SimulationShaderMaterial)
...
const FBOParticles: FC = () => {
  ...
  const simulationShader = useRef<typeof SimulationShader & SimulationUniforms>(null)
  const particlesShader = useRef<typeof ParticleAnimationShader & ParticleUniforms>(null)
      ...
      {createPortal(
        <mesh ref={meshRef}>
          <SimulationShader
            ...
            uPositions0={null}
            uPositions1={null}
            onBeforeCompile={(shader: { uniforms: Record<string, { value: DataTexture | null }> }) => {
              if (!shader) return
          ...
          <bufferGeometry>
            <bufferAttribute
              ...
              args={[squareGeometryPositions, 3]}
